...
namespace gre
{
	struct Matrix2x2
	{
		...
		static Matrix2x2 CreateScale(const float scale);
		static Matrix2x2 CreatetRotation(float radians);
	...
	inline Matrix2x2 Matrix2x2::operator*(const Matrix2x2& mx)
	{
		...
		result[0][0] = rows[0][0] * mx[0][0] + rows[0][1] * mx[1][1];
		result[0][1] = rows[0][0] * mx[0][0] + rows[0][1] * mx[1][1];
	...
	inline Matrix2x2 Matrix2x2::CreateScale(const float scale)
	{
		...
		mx[0] = Vector2{ 0.0f, scale };

		return mx;
	}

	inline Matrix2x2 Matrix2x2::CreatetRotation(float radians)
	{
		Matrix2x2 mx;
